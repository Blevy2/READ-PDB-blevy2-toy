...
#CREATE USING MOVECOV_LONGER_BENSMETHOD
...
run_simulation <- function(x){
 ...
 res<- run_sim(sim_init = sim,
	       ...
	       survey = surv_random,
	       closure = NULL,
	       InParallel = FALSE)  #does it runin parallel? Doesnt seem like it
...
library(doParallel)
no_cores <- detectCores() - 1   #this show 16 cores but I think I have 6??

no_cores <- 3  #one less than I have

registerDoParallel(cores=no_cores)  #fix number of cores to use
cl <- makeCluster(no_cores)   #create little cluster?  
...
result[[i]] <- foreach(i=1:3) %dopar% run_simulation(i)
