...
no_cores <- 5  #one less than I have

registerDoParallel(cores=no_cores)  #fix number of cores to use
cl <- makeCluster(no_cores, type="FORK")   #create little cluster?  
registerDoParallel(cl)
result <- foreach(i = 1:100) %dopar% results(i)


res <- list()

res[[i]] <- run_sim(sim_init = sim,
	       pop_init = Pop,
	       move_cov = moveCov,
	       fleets_init = fleets,
	       hab_init = hab,
	       ...
	       survey = surv_random,
	       closure = NULL,
	       InParallel = TRUE, cores = 10)  #does it runin parallel? Doesnt seem like it
