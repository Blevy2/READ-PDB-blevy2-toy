...
stratum <- sort(unique(as.vector(hab$stratas)))
...
yearly_pop_plots <- ggplot(annual_pop_results, aes(x = year, y = data, group = 2)) + geom_point() + geom_line() + 
  facet_wrap(~pop, scales = "free_y") + expand_limits(y = 0)
print(yearly_pop_plots)
